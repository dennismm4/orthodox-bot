...
def quiz_menu_kb():
    rows = []
    if QUIZ_OPEN["1"]:
        rows.append([InlineKeyboardButton("Day 1 Quiz", callback_data="start_day_1_quiz")])
    rows.append([InlineKeyboardButton("Go Back", callback_data="main_menu")])
    rows.append([InlineKeyboardButton("Exit", callback_data="exit_home")])
    return InlineKeyboardMarkup(rows)
...
def admin_menu_kb():
    ...
    quiz_action = "Close" if QUIZ_OPEN["1"] else "Open"
    rows.append([
        InlineKeyboardButton(
            f"{quiz_action} Day 1 Quiz",
            callback_data="admin_toggle_quiz_1",
        )
    ])
...
def admin_status_text() -> str:
    ...
    quiz_status = "Open" if QUIZ_OPEN["1"] else "Closed"
    lines.append(f"Day 1 Quiz: {quiz_status}")
    return "\n".join(lines)
...
def health_text() -> str:
    lines = [
        ...
        f"Day 1 quiz status: {'Open' if QUIZ_OPEN['1'] else 'Closed'}",
        "",
    ]
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    dispatch = {
        ...
        "admin_toggle_quiz_1": toggle_quiz,
