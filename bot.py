...
async def imagecheck(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not os.path.isdir(base):
        await update.message.reply_text(
            "Image folder not found on this deployment.\n\n"
            f"Expected folder:\n{base}"
        )
        return
    ...
    for card in FLASHCARDS:
        card_id = card["id"]
        if not get_image_path(f"q_{card_id}"):
            missing.append(f"q_{card_id}")
        if not get_image_path(f"a_{card_id}"):
    ...
    for name in files:
        ...
        newest.append((os.path.getmtime(path), f"{name} - {modified}"))
    newest = [line for _, line in sorted(newest, reverse=True)[:8]]
